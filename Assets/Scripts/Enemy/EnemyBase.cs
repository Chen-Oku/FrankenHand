    ...
    protected virtual void Update()
    {
        ...
        if (spriteRenderer != null && Camera.main != null)
        {
            ...
            if (agent != null && agent.velocity.sqrMagnitude > 0.01f)
            {
                ...
                Vector3 toCamera = (Camera.main.transform.position - spriteRenderer.transform.position).normalized;
                toCamera.y = 0;
                moveDir.y = 0;

                // Si el ángulo entre la dirección de movimiento y la dirección a la cámara es mayor a 90°, está "de espaldas"
                float dot = Vector3.Dot(moveDir, toCamera);
